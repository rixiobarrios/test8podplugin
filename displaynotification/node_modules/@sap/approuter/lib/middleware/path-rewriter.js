...
function applyRewriteRule(req, route, routerConfig) {
  tracer.info('Route %s used for request url %s', JSON.stringify(route), req.url);
  let destination = route.destination;
  const useService = route.service ? true : false;
  let path;
  let serviceDestination;

  const url = dynamicRoutingUtils.getCoreUrl (req);
  ...
  if (destination) {
    ...
    if (route.destination === '*') {
      if (!process.env['DESTINATION_HOST_PATTERN']) {
        throw new Error('Route with dynamic destination on host level matched but DESTINATION_HOST_PATTERN env. configuration missing');
      }
      ...
      if (!destination) {
        tracer.error('Failed to determine destination for host level destination - check DESTINATION_HOST_PATTERN');
        return;
      }

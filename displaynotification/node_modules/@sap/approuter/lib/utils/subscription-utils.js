...
function checkCertificate(req,credentials, cb){
  ...
  let subj;
  try {
    subj = JSON.parse(credentials.callback_certificate_subject);
  } catch (err){
...
function getSaaSRegistryCallbackPath(credentials){
  ...
  if (!credentials){
    credentials = vcapUtils.getServiceCredentialsByLabel('saas-registry');
  }
  if (!credentials){
    return callBackPath;
  }
...
function getSMSCallbackPath(credentials){
  ...
  if (!credentials){
    credentials = vcapUtils.getServiceCredentialsByLabel('subscription-manager');
  }
  if (!credentials){
    return callBackPath;
  }
  try {
    var appUrlsJson = JSON.parse(credentials.app_urls);

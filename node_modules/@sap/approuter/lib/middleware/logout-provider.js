...
function resolveLogoutRedirectUrl(req, cb) {
  ...
  passportUtils.getAuthConfig(req,req.authenticationType, function (err, authConfig) {
    ...
    var path = req.authenticationType === 'ias' ? '/oauth2/logout' : '/logout.do';
    var redirectUrl = urlUtils.join(authConfig.url, path);
    if (!logoutPage) {
    ...
    if (uri.protocol && uri.hostname) {
      return cb(null, redirectUrl + '?redirect=' + logoutPage + '&client_id=' + authConfig.clientid);
    }
    ...
    if (req.authenticationType === 'ias'){
      var logoutPageUrl = encodeURIComponent(urlUtils.join(appRouterUrl, logoutPage));
      cb(null, redirectUrl + '?post_logout_redirect_uri=' + logoutPageUrl + '&id_token_hint=' + req.idToken);
    } else {
      cb(null, redirectUrl + '?redirect=' + urlUtils.join(appRouterUrl, logoutPage) + '&client_id=' + authConfig.clientid);
    }
  });
}